...
BASE_PATH = os.getcwd()
root = os.path.join(BASE_PATH, 'data/widerface/widerface')
...
def wider2face(phase='val', ignore_small=0):
	data = {}
	with open('{}/{}/combined_label.txt'.format(root, phase), 'r') as f:
		lines = f.readlines()
		for line in tqdm(lines):
			line = line.strip()
			if '#' in line:
				path = '{}/{}/images/{}'.format(root, phase, line[2:])
				# print(line)
				img = cv2.imread(path)
...
if __name__ == '__main__':
	...
	for idx, data in enumerate(datas.keys()):
		...
		out_img = os.path.join(BASE_PATH, 'data/widerface/val/images/{}'.format(pict_name))
		out_txt = os.path.join(BASE_PATH, 'data/widerface/val/labels/{}.txt'.format(os.path.splitext(pict_name)[0]))
