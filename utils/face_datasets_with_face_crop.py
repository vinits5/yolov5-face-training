...
class LoadFaceImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < hyp['prob_crop']:
                img, labels, cropped = _crop_face(img, labels)
            if not cropped:
                img, labels, cropped_img = _crop(img, labels)
            h, w, _ = img.shape
            ...
            if labels.size > 0 and True:
                ...
                # labels[:, 8] = (labels[:, 8] / h)
                # labels[:, 9] = (labels[:, 9] / w)
                # labels[:, 10] = (labels[:, 10] / h)
                # labels[:, 11] = (labels[:, 11] / w)
                # labels[:, 12] = (labels[:, 12] / h)
                # labels[:, 13] = (labels[:, 13] / w)
                # labels[:, 14] = (labels[:, 14] / h)

                labels[:, 5] = np.where(labels[:, 5] == -1, -1, labels[:, 5] / w)
                labels[:, 6] = np.where(labels[:, 6] == -1, -1, labels[:, 6] / h)
                labels[:, 7] = np.where(labels[:, 7] == -1, -1, labels[:, 7] / w)
                labels[:, 8] = np.where(labels[:, 8] == -1, -1, labels[:, 8] / h)
                labels[:, 9] = np.where(labels[:, 9] == -1, -1, labels[:, 9] / w)
                labels[:, 10] = np.where(labels[:, 10] == -1, -1, labels[:, 10] / h)
                labels[:, 11] = np.where(labels[:, 11] == -1, -1, labels[:, 11] / w)
                labels[:, 12] = np.where(labels[:, 12] == -1, -1, labels[:, 12] / h)
                labels[:, 13] = np.where(labels[:, 13] == -1, -1, labels[:, 13] / w)
                labels[:, 14] = np.where(labels[:, 14] == -1, -1, labels[:, 14] / h)
            ...
            if labels.size > 0:
                ...
                # labels[:, 5] = (ratio[0] * w * labels[:, 5] + pad[0])
                # labels[:, 6] = (ratio[1] * h * labels[:, 6] + pad[1])
                # labels[:, 7] = (ratio[0] * w * labels[:, 7] + pad[0])
                # labels[:, 8] = (ratio[1] * h * labels[:, 8] + pad[1])
                # labels[:, 9] = (ratio[0] * w * labels[:, 9] + pad[0])
                # labels[:, 10] = (ratio[1] * h * labels[:, 10] + pad[1])
                # labels[:, 11] = (ratio[0] * w * labels[:, 11] + pad[0])
                # labels[:, 12] = (ratio[1] * h * labels[:, 12] + pad[1])
                # labels[:, 13] = (ratio[0] * w * labels[:, 13] + pad[0])
                # labels[:, 14] = (ratio[1] * h * labels[:, 14] + pad[1])

                labels[:, 5] = np.where(labels[:, 5] == -1, -1, (ratio[0] * w * labels[:, 5] + pad[0]))
                labels[:, 6] = np.where(labels[:, 6] == -1, -1, (ratio[1] * h * labels[:, 6] + pad[1]))
                labels[:, 7] = np.where(labels[:, 7] == -1, -1, (ratio[0] * w * labels[:, 7] + pad[0]))
                labels[:, 8] = np.where(labels[:, 8] == -1, -1, (ratio[1] * h * labels[:, 8] + pad[1]))
                labels[:, 9] = np.where(labels[:, 9] == -1, -1, (ratio[0] * w * labels[:, 9] + pad[0]))
                labels[:, 10] = np.where(labels[:, 10] == -1, -1, (ratio[1] * h * labels[:, 10] + pad[1]))
                labels[:, 11] = np.where(labels[:, 11] == -1, -1, (ratio[0] * w * labels[:, 11] + pad[0]))
                labels[:, 12] = np.where(labels[:, 12] == -1, -1, (ratio[1] * h * labels[:, 12] + pad[1]))
                labels[:, 13] = np.where(labels[:, 13] == -1, -1, (ratio[0] * w * labels[:, 13] + pad[0]))
                labels[:, 14] = np.where(labels[:, 14] == -1, -1, (ratio[1] * h * labels[:, 14] + pad[1]))


        if self.augment and not cropped:
            # Augment imagespace
            if not mosaic:
                if random.uniform(0, 1) < hyp['prob_aug']:
        ...
        if nL:
            ...
            labels[:, [5, 7, 9, 11, 13]] = np.where(labels[:, [5, 7, 9, 11, 13]] == -1, -1, labels[:, [5, 7, 9, 11, 13]] / img.shape[1])
            labels[:, [6, 8, 10, 12, 14]] = np.where(labels[:, [6, 8, 10, 12, 14]] == -1, -1, labels[:, [6, 8, 10, 12, 14]] / img.shape[0])
        ...
        if self.augment:
            ...
            if random.random() < hyp['flipud']:
                ...
                if nL:
                    ...
                    labels[:, 6] = np.where(labels[:,6] == -1, -1, 1 - labels[:, 6])
                    labels[:, 8] = np.where(labels[:, 8] == -1, -1, 1 - labels[:, 8])
                    labels[:, 10] = np.where(labels[:, 10] == -1, -1, 1 - labels[:, 10])
                    labels[:, 12] = np.where(labels[:, 12] == -1, -1, 1 - labels[:, 12])
                    labels[:, 14] = np.where(labels[:, 14] == -1, -1, 1 - labels[:, 14])
            ...
            if random.random() < hyp['fliplr']:
                ...
                if nL:
                    ...
                    labels[:, 5] = np.where(labels[:, 5] == -1, -1, 1 - labels[:, 5])
                    labels[:, 7] = np.where(labels[:, 7] == -1, -1, 1 - labels[:, 7])
                    labels[:, 9] = np.where(labels[:, 9] == -1, -1, 1 - labels[:, 9])
                    labels[:, 11] = np.where(labels[:, 11] == -1, -1, 1 - labels[:, 11])
                    labels[:, 13] = np.where(labels[:, 13] == -1, -1, 1 - labels[:, 13])
...
def generate_random_scales(SCALES):
    scale1 = random.choice(SCALES)/100.0
    scale2 = random.choice(SCALES)/100.0
    scale3 = random.choice(SCALES)/100.0
    scale4 = random.choice(SCALES)/100.0

    if scale1 < 0 and scale3 < 0 and scale2 > 0 and scale4 > 0:
        return generate_random_scales(SCALES)
    else:
        return scale1, scale2, scale3, scale4

def find_roi(bbox, SCALES):
    scale1, scale2, scale3, scale4 = generate_random_scales(SCALES)

    x1 = bbox[0]+scale1*(bbox[2]-bbox[0])
    x2 = bbox[2]+scale2*(bbox[2]-bbox[0])
    y1 = bbox[1]+scale3*(bbox[3]-bbox[1])
    y2 = bbox[3]+scale4*(bbox[3]-bbox[1])
    ...
    if height>width:
        ...
        if scale1 < 0:
            ...
            if scale2 > 0:
                ...
                # return find_roi(bbox, SCALES)
                return find_roi_tight_crop(bbox)
    if width>height:
        # check scale3 and scale4.
        if scale3 < 0:
            # pad along top
            roi[1] -= pad
        else:
...
def find_roi_tight_crop(bbox):
    SCALES = [0, 10, 20]
    scale1 = random.choice(SCALES)/100.0        # left cropping.
    SCALES = [-20, -10, 0]
    scale2 = random.choice(SCALES)/100.0        # right cropping.
    SCALES = [0, 10, 20]
    scale3 = random.choice(SCALES)/100.0        # top cropping.
    SCALES = [-20, -10, 0]
    scale4 = random.choice(SCALES)/100.0        # bottom corpping.
...
def _crop_face(image, targets):
    ...
    SCALES = [-40, -20, 0, 20, 40]

    for idx in range(boxes.shape[0]):
        if (boxes[idx, 2] - boxes[idx, 0]) > T1 and (boxes[idx, 3] - boxes[idx, 1]) > T2:
            bbox = boxes[idx]
            
            roi = find_roi(bbox, SCALES)
            roi = roi.copy().astype('int')
            if (roi<0).any():
                neg_dim = np.where(roi<0)[0][0]
                delta = abs(roi[neg_dim])
                roi[neg_dim+2] += delta
                roi[neg_dim] = 0
            ...
            if (roi[2]>width or roi[3]>height):
                if roi[2]>width: 
                    pos_dim = 2
                    delta = abs(roi[pos_dim] - width)
                else: 
                    pos_dim = 3
                    delta = abs(roi[pos_dim] - height)
                roi[pos_dim-2] -= delta
                roi[pos_dim] = image.shape[pos_dim-2]
                if (roi<0).any():
                    # import pdb; pdb.set_trace()
                    continue
            ...
            iou = matrix_iou(boxes.copy(), roi[np.newaxis].copy())
            mask_a = (iou>0).all(axis=1)
            ...
            # landm
            lc = (landms_t==-1).all(axis=1)
            lc = (lc==True).all(axis=1)
            landms_t[:, :, :2] = landms_t[:, :, :2] - roi[:2]
            landms_t[np.where(lc==True)[0]] = -1  

            landms_t = landms_t.reshape([-1, 10])
            targets_t = np.hstack((np.zeros((landms_t.shape[0], 1)), boxes_t, landms_t))

            if (image_t.shape[0] < 1 or image_t.shape[1]<1):
                continue
            return image_t, targets_t, not cropped
        else:
            continue
    return image, targets, cropped

# def _crop_face(image, targets, img_dim=800):
#     T1, T2 = 100,100
#     boxes = targets[:, 1:5].copy()
#     boxes[:, 2:] -= boxes[:, :2]        # xyxy to xywh
#     landm = targets[:, 5:].copy()
...
#     SCALES = [-60, -40, -20, 20, 40, 60]

#     for idx in range(boxes.shape[0]):
#         if boxes[idx, 2] > T1 and boxes[idx, 3] > T2 and (landm[idx] != -1).all():
#             roi = boxes[idx]
#             max_dim = np.argmax(roi[2:])+2      # either width or height.

#             # # Update bbox to cropped face.
#             # boxes_t = boxes[idx:idx+1].copy()
#             # boxes_t[:, :2] -= roi[:2]

#             # # Update Landmarks to cropped face.
#             # landms_t = landm[idx:idx+1].copy()
#             # landms_t = landms_t.reshape([-1, 5, 2])
#             # landms_t[:, :, :2] = landms_t[:, :, :2] - roi[:2]

#             # # Shift the ROI.
#             # scale = random.choice(SCALES)
#             # roi[max_dim-2] -= (scale/100.0)*roi[max_dim]
#             # boxes_t[:, max_dim-2] += (scale/100.0)*roi[max_dim]
#             # landms_t[:, :, max_dim-2] += (scale/100.0)*roi[max_dim]

#             scale = random.choice(SCALES)
#             roi_new = roi.copy()
#             roi_new[max_dim-2] -= (scale/100.0)*roi[max_dim]    # update top-left coordinates.

#             iou = matrix_iou(boxes.copy(), roi_new[np.newaxis].copy())
#             mask_a = (iou>0).all(axis=1)
#             boxes_t = boxes[mask_a].copy()
#             landms_t = landm[mask_a].copy()
#             landms_t = landms_t.reshape([-1, 5, 2])

#             # Update bboxes and landmarks.
#             boxes_t[:, :2] -= roi[:2]
#             boxes_t[:, max_dim-2] += (scale/100.0)*roi_new[max_dim]
#             landms_t[:, :, :2] = landms_t[:, :, :2] - roi[:2]
#             landms_t[:, :, max_dim-2] += (scale/100.0)*roi_new[max_dim]

#             # Crop roi from original image.
#             roi_new = roi_new.astype('int')
#             roi_new[2:] += roi_new[:2]
#             roi_new = np.where(roi_new<0, 0, roi_new)
#             image_t = image[roi_new[1]:roi_new[3], roi_new[0]:roi_new[2]]

#             boxes_t[:, 2:] += boxes_t[:, :2]        # xywh to xyxy
#             landms_t = landms_t.reshape([-1, 10])
#             targets_t = np.hstack((np.zeros((landms_t.shape[0], 1)), boxes_t, landms_t))
#             return image_t, targets_t, not cropped
#         else:
